package br.com.sisdb.greendogdelivery.greendogdelivery.controller;
...
import br.com.sisdb.greendogdelivery.greendogdelivery.model.Pedido;
import br.com.sisdb.greendogdelivery.greendogdelivery.repository.ClienteRepository;
import br.com.sisdb.greendogdelivery.greendogdelivery.repository.ItemRepository;
import br.com.sisdb.greendogdelivery.greendogdelivery.repository.PedidoRepository;
...
public class PedidoController {
	...
	public ModelAndView create(@Valid Pedido pedido,BindingResult result,RedirectAttributes redirect) {
		...
		if (pedido.getId() != null) {
			Pedido pedidoParaAlterar = pedidoRepository.getOne(pedido.getId());
			Cliente c = clienteRepository.getOne(pedidoParaAlterar.getCliente().getId());
			...
			Cliente c = clienteRepository.getOne(pedido.getCliente().getId());
			double valorTotal = 0;
			for (Item i : pedido.getItens()) {
				valorTotal +=i.getPreco();
			}
	...
	public ModelAndView remover(@PathVariable("id") Long id,RedirectAttributes redirect) {
		...
		Pedido pedidoParaRemover = pedidoRepository.getOne(id);

		Cliente c = clienteRepository.getOne(pedidoParaRemover.getCliente().getId());
